...
export default function FeatureSummary({
  ...
  const linkBody = (l: Action) => (
    <>
      ...
      <span
        className={`font-bold ${primary} ${darkMode ? "group-hover:text-blue-200" : "group-hover:text-primary"
          }`}
      >
        {l.action}
      </span>
  ...
  return (
    <div
      className={`comp_summary ${darkMode ? "dark-mode" : ""} ${centerAlign ? "text-center" : "text-left"
        } ${className}`}
    >
      {label &&
        (small ? (
      ...
      <div
        className={`flex gap-5 ${centerAlign ? "items-center" : "items-start w-full"
